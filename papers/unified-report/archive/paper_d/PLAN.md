...
Paper D depends on Policy to Knowledge through immutable files, not Python imports into a mutable sibling checkout and not a shared database. A one-way adapter copies approved stage artifacts into `papers/unified-report/archive/paper_d/inputs/p2k/<run_id>/`, hashes them, validates their schemas, and emits Paper D candidate objects without changing the upstream bytes.
...
- a `studies/registry.yaml` entry with `claim_authorization: false` (Section 19.1), plus regenerated
  `studies/README.md` and `papers/unified-report/PAPERS_INDEX.md`;
...
(`papers/unified-report/archive/paper_c/specialize_then_align/`): a per-study `.gitignore`, `locks/` for lock artifacts,
...
```text
papers/unified-report/archive/paper_d/
...
The Paper D code should reject writes outside `papers/unified-report/archive/paper_d/` except for an explicit, read-only import step that copies and hashes approved external source artifacts into the isolated namespace.
...
`gitignore_asymmetry`. Because `papers/unified-report/archive/paper_d/inputs/` will hold Fannie Mae source bytes and
...
and `papers/unified-report/PAPERS_INDEX.md` are **generated from it**, `make check-registry` validates it and asserts
...
- study_id: paper_d_policy_to_guard_v1
  ...
  verification_command: "test -f papers/unified-report/archive/paper_d/PLAN.md"
  expected_verification_status: expected_pass
  code_root: papers/unified-report/archive/paper_d/
  protocol_paths: [papers/unified-report/archive/paper_d/PLAN.md, papers/unified-report/archive/paper_d/PROTOCOL.md]
  status_path: papers/unified-report/archive/paper_d/STATUS.md
  environment_path: papers/unified-report/archive/paper_d/environment/requirements.txt
...
- **Registry and indexes must stay in sync.** After editing the registry, run
  ...
  `papers/unified-report/PAPERS_INDEX.md`; `make check-registry` fails if they go stale.
