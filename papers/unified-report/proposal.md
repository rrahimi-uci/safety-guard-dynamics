...
**Correction (2026-07-30).** An earlier version of this section reported these deltas from an estimator that averaged the five seed score vectors before computing the metric, which measures a five-run ensemble rather than one trained guard and made the displayed arithmetic fail (`.948 − .741 = .207`, not the `+0.185` printed). The estimator is now the mean of per-seed paired differences with rows *and* seeds resampled. Two consequences matter for §20: the per-cell intervals widen, and after a Holm correction across the 12 represented-source SFT cells **none** stays significant — 4 of 12 clear zero nominally, 0 after adjustment. The claim-bearing quantity is now a pre-specified aggregate: equal-source mean ΔTPR@5%FPR = **+0.083 [+0.008, +0.163]**, ΔAP = +0.039 [+0.014, +0.072]. This *strengthens* §20.12's argument that per-cell post-selection intervals cannot carry a claim at this sample size.

On represented sources the small tuned guards **rank better than the frontier reference on average** (the aggregate above), and the `prompt_injections` cell is a ranking rather than a threshold advantage (AUROC 0.9928 against 0.8731) — but it is the largest of twelve and is exploratory only. On transfer sources the ordering reverses and ordinary SFT is what costs the guard its position; the strongest transfer guard in the panel is an *untuned* base. The `jailbreakbench` column supports no comparison at all: the reference's own TPR there is collapsed inside a tie block of its coarse integer risk score, so differences against it are uninterpretable.
