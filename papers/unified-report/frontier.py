...
that yields roughly 50--65 distinct values over 2,275 rows. Heavy ties LIMIT the RESOLUTION
of the frontier ranking: within a tie block the true ordering is unobserved. That is a
statement about precision, not about direction -- a finer score could resolve a tie block
favourably or unfavourably, so it does NOT follow that a frontier advantage is conservative.
Earlier versions of this module and of the report's captions asserted that direction; they
were wrong to. Report the limited resolution and leave the sign of its effect open.
...
def emit_table(data: dict | None = None) -> str:
    ...
    if bf and bl:
        ...
        note += ("The frontier score is a coarse integer 0--100 risk with heavy ties, which "
                 "caps how finely its ranking can be resolved. That limits precision without fixing a direction: a finer score could order a tie block either way, so these deltas should not be read as conservative.}")
    ...
    cap = ("Frontier hosted guardrails against this report's local guards on ExpGuard "
           ...
           "Responses API exposes for reasoning models. SFT rows are the \\emph{in-env} "
           "$\\beta{=}0$ "
...
def emit_serving_table(data: dict | None = None) -> str:
    ...
    cap = ("What the frontier accuracy in \\Cref{tab:frontier} costs to serve. Local P50 is the "
           ...
           "owning a GPU, which is what the self-hosted column elides. The trade is roughly two "
           "orders of magnitude in median latency against about \\emph{halving} the misses: at a "
           "matched $5\\%$ budget the hosted model misses $10.4\\%$ of unsafe prompts against the "
           "best small base's $21.3\\%$ ($2.0\\times$ fewer) and the best $32$B open base's "
           "$17.0\\%$ ($1.6\\times$ fewer). An earlier version of this caption said \\emph{one "
           "order of magnitude} fewer misses, which was simply wrong. Three properties also do "
           "not appear in any column: prompts leave the "
