...
def main(argv=None) -> int:
    ...
    inputs = sorted(p.name for p in GEN.glob("*.tex") if p.name != "repro_macros.tex")
    ...
    failed_inputs = [k for k in failed if k.split(":")[-1].endswith(".tex")]
    ...
    # and let the prose read the macros.
    (GEN / "repro_macros.tex").write_text("\n".join([
