...
\fancyhead[C]{\footnotesize\itshape Safety Benchmark Gains Do Not Guarantee Safety Transfer}
...
\input{generated/lowfpr_macros}      % Act I re-read over FPR [0,.05]: \LowFpr*
\input{generated/lowfpr_kl_macros}   % the KL dial in the same region: \LowFprKl*
\input{generated/teaser_macros}      % Figure 1 caption: 2-dp \Teaser* forms of the panel deltas

\title{\textbf{Safety Benchmark Gains Do Not Guarantee Safety Transfer}\\[3pt]
\large A Comprehensive Study of Fine-Tuning Small Language Model Safety Guards for High-Compliance and General Safety Domains}
...
% No \clearpage here. There used to be one, and it produced a BLANK page between the figure
% and the ledger: `float`'s [H] already emits its own page break when the float does not fit
% in the space remaining, so the explicit \clearpage made two breaks where one was wanted.
% The spread still reads figure-then-ledger on consecutive pages, which is the whole point of
% pinning both with [H]; it just no longer wastes a sheet doing it.
...
inside it), Q2 its top-right, Q3 its bottom-left, Q4 its bottom-right. Q1b, Q1c and Q4a sharpen their
neighbours and have no panel of their own --- Q1c in particular re-prices Q1 rather than adding
evidence to it, and is the row to read before quoting any of these deltas as a deployment number. Nothing later in the report is claimed at a stronger
...
Q1c. Does the \emph{metric} understate it? &
The same eight cells re-read as partial AUC over FPR $[0,\LowFprBudget]$ and as recall at that
budget, instead of over the whole ranking &
\textbf{Yes, by two to three times --- but no cell flips sign.} Transfer $\LowFprTransApDelta$ AP
vs.\ $\LowFprTransPAucDelta$ pAUC; represented $\LowFprRepApDelta$ vs.\ $\LowFprRepPAucDelta$ &
Retro. & \S\ref{sec:actI-lowfpr} \\
...
off-source (Act~I). The choice of \emph{metric} co-produces the verdict too, in the same direction:
averaging precision over the whole ranking makes the trade look milder than it is, and re-reading the
identical rows inside a $\LowFprBudget$ alarm budget roughly triples the transfer cost without flipping
a single cell (\Cref{sec:actI-lowfpr}); keeping the base in an output-space average
...
unequal alarm rates hides its sign. Report the trade in the region you will deploy in: the same cells
read over FPR $[0,\LowFprBudget]$ give $\LowFprTransPAucDelta$ transfer, not $\LowFprTransApDelta$
(\Cref{tab:lowfpr}). \\
\rowcolor{white}
...
Read inside that budget rather than over the whole ranking, the same paired comparison is
$\LowFprRepAmplification\times$ larger on the represented side and $\LowFprTransAmplification\times$
on transfer, with \LowFprSignFlips{} checkpoints changing sign --- so the metric that produced the
headline was understating it, and the one base whose transfer improves on macro-AP does not improve
at a deployable alarm rate.
