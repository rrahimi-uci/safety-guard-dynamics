...
So the answer is no: ensembling small guards does not beat the hosted model, and the cheap
...
external held-out prompts, and the best tuned configuration anywhere in this study
...
reference: \HtwoBestName{} --- a \emph{1.5B} model --- reaches \HtwoBestTpr{} on
\HtwoBestSource{} against the reference's \HtwoRefTpr, a paired difference of
\HtwoBestDeltaTpr{} \HtwoBestDeltaTprCI{}, and it is a \emph{ranking} win rather than a
threshold placement (AUROC \HtwoBestAuroc{} against \HtwoRefAuroc). \HtwoNSig{} of
\HtwoNSftCells{} panel-SFT cells on represented sources clear zero at the $95\%$ level. On
...
(\Cref{sec:decontamination}) puts $1.6\%$--$5.0\%$ of each represented split within
...
from $0.787$ to $0.819$; $20\%$ reaches $0.842$, which is \emph{half} the distance to the
hosted model's \FrontierBestTpr; $30\%$ reaches $0.856$. The curve is steep early because the
...
\Cref{sec:actIV-mortgage} is exactly the question that has to be answered first.
...
inverts and a \HtwoBestName{} guard beats it outright (\HtwoBestDeltaTpr{} \HtwoBestDeltaTprCI{};
\Cref{sec:actIII-h2h}). So the gap is not a capability ceiling to be closed by a larger student --- it is
...
\rowcolor{guidealt}
\textbf{\textcolor{tkgreenedge}{8.}~The frontier gap is a property of the \emph{regime}, not of model size: a 1.5B guard beats a hosted frontier model on sources it represents, and loses on sources it does not.} &
On five corpora scored by both, at a matched \HtwoBudget{} alarm budget: \HtwoBestName{} SFT reaches
\HtwoBestTpr{} on \HtwoBestSource{} against \HtwoRef{}'s \HtwoRefTpr{} (\HtwoBestDeltaTpr{}
\HtwoBestDeltaTprCI{}, AUROC \HtwoBestAuroc{} vs.\ \HtwoRefAuroc{}), \HtwoNSig{} of \HtwoNSftCells{}
represented cells clearing zero; on transfer sources the hosted model leads and the best local guard is
an \emph{untuned} base (\Cref{sec:actIII-h2h}). Retrospective, inspected panel. &
