...
locked analyses), and the dual-labeled mortgage benchmark ships frozen and checksummed. Those counts are emitted by the harness rather than typed here.
\end{abstract}
...
\begin{enumerate}[leftmargin=1.6em,itemsep=1pt]
  ...
  \item \textbf{Should you run a small guard at all?} (\Cref{sec:actIII-frontier}) --- the same guards priced against a hosted frontier model on identical external rows, in accuracy, latency and cost.
\end{enumerate}
Two further bodies of evidence sit inside that arc rather than beside it: an
\emph{analysis-preregistered} ten-checkpoint study asking whether the same specialization hits guards
that are \emph{already} purpose-built (\Cref{sec:adaptation}), and a Qwen3 scale ladder and released-guard
panel that test whether size or an off-the-shelf guard is the cheaper escape (\Cref{sec:frontier-routes}).
...
\noindent These are the four \emph{base} checkpoints
...
\HtwoAggDeltaAp{} \HtwoAggDeltaApCI{} in AP; both exclude zero.
...
The weighting choice matters, so we give the alternatives rather than only the one we report.
Weighting sources equally gives \HtwoAggDeltaTpr{}; weighting all twelve cells equally gives the
same value; weighting sources by their row counts gives $+0.049$, roughly \emph{half}, because the
largest source (\code{toxicchat}, $n=451$) carries the smallest effect; and including the
\emph{base} arms alongside the tuned ones gives $-0.265$, i.e.\ the sign reverses. That last
figure is the important one to keep in view: the represented-source advantage is a property of
\emph{tuned} guards specifically, which is the specialization result of Act~I seen from the other
side, not a general statement that small guards beat hosted ones. On
...
\section{Synthesis: one lesson at every scale we measured, and what to do about it}
\label{sec:synthesis}
...
figures in \Cref{tab:latency}. The hosted path adds a network round-trip --- $1{,}553$\,ms median against
tens of milliseconds --- to \emph{every} request; it bills a per-token fee that scales with traffic; and,
decisively for a regulated domain, it sends every prompt to a third party.
...
are committed outputs of their own locked analyses that are not yet wired into the byte-check. Every count in this paragraph is emitted by the harness itself
(\code{\textbackslash Repro*}), because they were previously typed by hand and went stale: an earlier
version of the abstract and of this paragraph quoted ``12 of the 24'' after the artifact surface had
already grown past it. Three limits are worth stating plainly rather than leaving to a
...
benchmark that measures it, the objective that tuned it, the threshold that reads it, the domain
it is deployed into, and --- the axis this report added last --- \emph{whether the traffic it is
asked about is traffic it was trained to expect}. This report is an attempt to say that precisely
enough to act on, by varying each of those five factors while holding the others fixed.

That fifth axis is what makes the sourcing question answerable. Priced against a hosted frontier
model on identical external rows, a small guard is the worse ranker on unfamiliar prompts and the
better one on sources its training represents (\Cref{sec:actIII-frontier}); the measured serving gap
is roughly two orders of magnitude in median latency against about halving the misses. So the
deployment question is not \emph{which guard is better} --- neither is, unconditionally --- but
\emph{what share of your traffic you can characterise in advance}, and how you route the rest. We
price one such router here and are explicit that the more attractive one, keyed to unfamiliarity
rather than to score margin, is a hypothesis we did not test.
