...
purposively chosen corpora, and resampling that source set gives \HtwoAggSrcResampledCI{}; the
\FrontierGainOverBase{} bar below it is a paired comparison on external expert-annotated rows. Two
...
not carry across the two arms (mortgage does not separate the panel at all) &
...
\textbf{If you have five minutes:} \Cref{fig:teaser} (p.~\cpageref{fig:teaser}) is the four answers as
one picture, and \Cref{tab:glance} (p.~\cpageref{tab:glance}) is every headline number with its
...
model's self-chosen operating point. The rightmost two columns are where this report's remaining
claim lives; the columns to their left are where it does not.}
\label{tab:closest}
...
the two arms}: the mortgage AP$\cdot$D intervals do not separate the panel at all, while on ExpGuard
SmolLM3-3B is highest (\Cref{tab:baseline,tab:expguard}).}
...
values over $2{,}275$ rows, and those ties cap how finely AP can resolve a ranking. That bounds
...
scale, seed-ensembling and a fitted stack move up but stall around $40$--$60\%$ of the way, and the
...
run; it exits nonzero unless every covered artifact matches. One limit is measured rather than
...
kept here so the record shows what the numbers above are worth. \textbf{(i)} Ten generated inputs sat
...
table is uncovered. \textbf{(ii)} Figures were regenerated and never compared. They are now rendered
into a scratch directory and byte-compared like everything else. \textbf{(iii)} Verification was not
