...
$-0.043$. Because a null family only pulls an equal-family mean toward zero, this cannot manufacture
support: it makes RQ1 harder to clear and makes RQ2's failure look milder than it is. Both verdicts
therefore stand, and the reported magnitudes are conservative --- but they are $20\%$ smaller than the
...
\code{stop\_narrow\_gates} fallback does authorize omitting it; but that fallback is conditional, and
the condition it names (fewer than three purpose-built families passing preflight) did not obtain ---
five did. The operative obstacle is the family taxonomy actually used in the analysis, which places
Qwen2.5-1.5B, Qwen3-4B, Qwen3Guard-Gen-0.6B and Qwen3Guard-Gen-4B in a single \code{qwen} family. A
family that spans both starting types cannot be assigned to either side of $\Gamma$, so the estimand is
ill-defined under this taxonomy. Splitting Qwen by starting type would make it computable, but that is
an analytic choice made after seeing the scores and no bootstrap exists for it, so no $\Gamma$ is quoted
here. What this report supports is therefore per-checkpoint and per-family movement, not a pooled
cross-type contrast.
