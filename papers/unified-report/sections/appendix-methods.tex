...
The paired studies in this report --- SFT, composition, and the zero-shot domain baselines --- reuse one fixed, purposively chosen panel of four checkpoints and one
...
most, the one composition helps least, and the numerically highest-ranking zero-shot mortgage guard
(within overlapping CIs --- \Cref{tab:baseline} does not license a ranking). It is \emph{not} the most
protected-token-invariant guard on that split: its $\Delta_{\mathrm{context}}=0.000$ is a saturation
artifact of the probability scale, and read on the raw margin its gap is the second largest on the panel
(\Cref{sec:actIV-mortgage-pairs}).
...
\label{tab:datasets}
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.40\linewidth} >{\raggedright\arraybackslash}p{0.30\linewidth} >{\raggedright\arraybackslash}p{0.12\linewidth} c@{}}
\toprule
...
manifest against the current v2 transfer suite. That audit has since been run
(\code{experiments/audit\_overlap\_lineage.py}, committed to \code{artifacts/overlap\_audit/}) and it is
clean: against the $1{,}200$-row manifest the v2 transfer suite shows zero exact and zero normalized
matches, zero \code{family\_id} or \code{upstream\_family\_id} collisions, nothing at $5$-gram containment
$\geq0.80$, and nothing at character-shingle Jaccard $\geq0.70$, so the transfer estimates need no
downward revision (\Cref{sec:decontamination}). What neither the gate nor the audit covers is
\emph{semantic} overlap: no embedding-space check is implemented, so a paraphrase sharing no $5$-gram with
any training row would pass both, and closing that needs an encoder pinned into the lock
(\Cref{sec:roadmap}).
...
\paragraph{Matched-FPR points are ROC summaries, not deployable thresholds.}
\label{sec:matched-fpr-limits}
Three tables in this report --- \Cref{tab:matchedfpr}, \Cref{tab:frontier} and \Cref{tab:h2h} --- place
every guard at a common $5\%$ false-alarm budget so that recalls are comparable. That budget is located
by taking a quantile of the \emph{evaluated} negatives and then measuring recall on the \emph{same}
rows. This is a legitimate and standard way to read a pair of ROC curves at one point, and the paired
bootstrap re-estimates that functional inside every replicate, so the intervals are internally
consistent. It is \emph{not} a threshold a production system could use, because placing it requires the
labels the system is trying to predict. The distinction matters because it changes what the number
means: ``TPR at an empirical matched-FPR ROC point'' is a statement about \emph{ranking quality} at a
comparable alarm rate, whereas a deployment claim would require the threshold to be fixed on a disjoint
calibration set, frozen, and then reported with its \emph{realized} FPR and TPR on untouched acceptance
rows. Only the calibration-only operating point above does that, and only for Acts~I--II. No
matched-FPR figure anywhere in this report should be read as an achievable deployment operating point,
and none of the deployment guidance in \Cref{tab:guidelines} rests on one.
...
unit is one incoming request carrying two \emph{separately assigned} labels, general-safety $G$ and
mortgage-policy $D$ (\Cref{sec:bg-mortgage}) --- separately assigned, but not empirically independent in
v1, where the empty G1/D0 cell nests $G$ inside $D$ (\Cref{sec:actIV-mortgage-composition}). Requests are grounded in de-identified, banded HMDA fact sheets --- never
...
provenance-tracked, \code{content\_family}-isolated split into the frozen release (\Cref{fig:pipeline};
that grouping key is narrower than pair-level isolation, and one protected pair does cross a split ---
\Cref{sec:actIV-mortgage-composition}). The benchmark
...
arm and a hosted-frontier reference point have since been run on the same rows and are central to
\Cref{sec:actIII-frontier} (\Cref{tab:frontier}). What is still future work is a \emph{dual-labeled}
finance/health construct with expert sign-off (\Cref{sec:roadmap}).
