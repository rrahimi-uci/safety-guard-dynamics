...
unit is one incoming request and whose defining move is to carry \emph{two separately assigned} labels ---
...
the two genuine single-token pairs its gap is $0.020$ --- inside the unsaturated guards' band rather than
an outlier above it (SmolLM3-3B $0.012$, SmolLM2-1.7B $0.024$; the $\Delta^{\mathrm{1tok}}$ column of
\Cref{tab:baseline}), so the apparent contrast is carried by the one non-minimal pair and not by the
guard. Exhaustive enumeration of all $27$ three-pair
